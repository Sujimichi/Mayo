...
Mayo is still very a work in progress.  Its pretty basic and not fit for general consumption.  That being said; I'm now running my full (normally 30 minute) test suite in 5 minutes.
...
Mayo will also randomize the tests to decrease the chance of one test always being run on the same client.  

Mayo does not need any change to be made to the project being tested, not even test env configs.  I felt that its important for this just to be a work spreader and to have no impact on the project.  


==Dividing the Load.

When used to run rpsec tests each client will be instructed to run a share of the *_spec.rb files.  So this is not a good solution when you only have 1 spec file!

When used to run cucumber features however mayo will not just split the files between the clients it divides the scenarios between them.  So one .feature file with several scenarios will be split over several clients.  
