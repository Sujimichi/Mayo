...
==The Mayo Solution
The notion behind it is very simple; Mirror the directory containing the files to be tested to several client machines.  Then run a command on the client and collect the response.  The command diriects the clients to do a portion of the whole task.
...
Before a client is sent a command to process it is first sent an updated copy of the directory over rsync.  This is why the clients need the servers' id_rsa.pub in their authorized_keys.  As each client completes its portion its results are collected and displayed by the server.  The server comunicates the commands and responces between the clients using Ruby TCPSockets.  
...
The goal of getting cucumber tests to run faster is not about computational power, its about the speed at which dumb browsers can take input.  Simply spliting features between two instances of cucumber (each on a separate virutal machine running on the same physical machine) will almost certainly halve the run time.  With 5 clients I've been able to bring a 30 minute test suite down to 6 minutes.  
